...
expectRedirect(r,'http://localhost/user/login?returnTo=http%3A%2F%2Flocalhost%2Fproject%2F9999%2Fcancel%3Fid%3D9999')
...
r = admin_session.get('http://localhost/project/cancel',allow_redirects=False)
...
r = admin_session.get('http://localhost/project/9999/cancel',allow_redirects=False)
expectRedirect(r,'http://localhost/project/')
...
# admin should be able to cancel his project
r = admin_session.get('http://localhost/project/1/cancel',allow_redirects=False)
expectRedirect(r,'http://localhost/project/1/view')

cursor.execute('SELECT * FROM projects WHERE id = 1')
expect(cursor.fetchone() == (1, None, 'admin-project', 'owned by admin', CANCELED))

# user should not be able to cancel admin's project
r = user_session.get('http://localhost/project/1/cancel',allow_redirects=False)
expectRedirect(r,'http://localhost/project/')
...
# user2 should be able to cancel his project
r = user_session.get('http://localhost/project/2/cancel',allow_redirects=False)
