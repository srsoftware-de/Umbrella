...
r = requests.get('http://localhost/task/1/open',allow_redirects=False)
expectRedirect(r,'http://localhost/user/login?returnTo=http%3A%2F%2Flocalhost%2Ftask%2F1%2Fopen%3Fid%3D1')
...
# task_id: absent
r = admin_session.get('http://localhost/task/open',allow_redirects=False)
...
# task_id: non-existig
r = admin_session.get('http://localhost/task/9999/open',allow_redirects=False)
expectRedirect(r,'http://localhost/task/');
...
r = user_session.get('http://localhost/task/1/open',allow_redirects=False)
expectRedirect(r,'http://localhost/task/');
...
r = user_session.get('http://localhost/task/2/open',allow_redirects=False)
...
expect((2,OPEN) in rows)
    
# task without redirect should be redirected to itself
r = admin_session.get('http://localhost/task/1/open',allow_redirects=False)
expectRedirect(r,'http://localhost/task/1/view');
